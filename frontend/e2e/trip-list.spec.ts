...
test.describe("trip list (/app)", () => {
  test("empty state shows the plan-a-trip CTA", async ({ page, request }) => {
    ...
    await page.goto("/app");
    await expect(page.getByRole("heading", { name: /your readings|my trips/i })).toBeVisible();
    // Empty-state copy uses one of these phrases.
    await expect(
      page
        .getByText(/the notebook.s empty|no trips yet|first trip|plan a new trip|plan a trip/i)
        .first(),
    ).toBeVisible();
    await expect(
      page.getByRole("link", { name: /new reading|plan a new trip/i }).first(),
    ).toBeVisible();
  });
  ...
  test("creating two trips renders both cards; clicking one navigates to detail", async ({
    ...
    for (const dest of ["Tokyo", "Osaka"]) {
      ...
      await page.getByLabel(/the place|destination/i).fill(dest);
